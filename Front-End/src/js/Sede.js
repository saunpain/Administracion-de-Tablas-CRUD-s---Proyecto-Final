...
function ObtenerSede(){
    fetch(baseUrl + "/sede/all").then( res => {
        res.json().then(json => {
            sede = json
            console.log(sede)
            ImprimirSede(sede)
...
function ImprimirSede(sede){
    ...
    sede.forEach(s => {
        contenedor.innerHTML += MapearSede(s)
        console.log(s)
    })
...
function MapearSede(s) {
    ...
    <td class="checkbox px-2 appearance-none border border-solid border-gray-300 rounded-full w-5 h-5 cursor-pointer checked:bg-gray-700">
        <input type="checkbox" class="ml-3.5 seleccionar" id="${s.cod_sede}" />
        <label for="${s.cod_sede}"></label>
    </td>
    <td class="border border-solid border-gray-300 text-center px-8 py-2 whitespace-nowrap text-gray-700">${s.cod_sede}</td>
    <td class="border border-solid border-gray-300 text-center px-8 py-2 whitespace-nowrap text-gray-700">${s.nombre_sede}</td>
    <td class="border border-solid border-gray-300 text-center px-8 py-2 whitespace-nowrap text-gray-700">${s.ubicacion}</td>
</tr>`
}
